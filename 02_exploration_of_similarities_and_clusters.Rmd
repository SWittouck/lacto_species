...
ggsave(paste0(dout, "/density_cni.png"), units = "cm", width = 15, height = 10)
```
...
ggsave(paste0(dout, "/density_ani.png"), units = "cm", width = 15, height = 10)
```
...
ggsave(paste0(dout, "/density_tetra.png"), units = "cm", width = 15, height = 10)
```
...
ggsave(paste0(dout, "/scatter_cni_ani.png"), units = "cm", width = 15, height = 10)
```
...
exclusivity %>%
  ...
  geom_line() +
  geom_point(aes(col = all_clusters_exclusive)) +
  scale_color_brewer(palette = "Paired") +
  theme_bw() +
  xlab("core nucleotide identity (CNI) cutoff") +
  ylab("number of non-exclusive clusters")
ggsave("results_v3/figure_S3_exclusivity.jpg", units = "cm", width = 20, height = 10)
```
...
ggsave(paste0(dout, "/figure_S2_clusters_cnis.jpg"), units = "cm", width = 20, height = 25)
```
...
ggarrange(
  ggarrange(
    fig_density_cni %>% give_letter("A"), 
    fig_density_ani %>% give_letter("B"), 
    ncol = 1, nrow = 2
  ),
  fig_scatter_cni_ani %>% give_letter("C"),
  nrow = 1, ncol = 2
)
ggsave(paste0(dout, "/figure_2.jpg"), units = "cm", width = 40, height = 20)
```
