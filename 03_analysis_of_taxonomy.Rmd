...
bind_rows(
  ...
  select(cluster, merger_split, names, n_genomes = n, min_cni_within, max_cni_between) %>%
...
clusters_tss <-
  ...
  mutate(
    species = case_when(
      ...
      certainty == "unidentified species" ~ str_c("Unidentified species", 1:n(), sep = " ")
    )
  ) %>%
  ungroup()
```
...
clusters_tss %>%
  mutate_at(
    "certainty", factor, 
    levels = c("certain", "best guess", "unidentified", "new species")
  ) %>%
  select(cluster, n_genomes, species_ncbi, n_16S, sixteen_s_hits, certainty, species) %>%
  arrange(certainty) %>%
...
ggsave(paste0(dout, "/figure_4.png"), units = "cm", width = 30, height = 40)
```
