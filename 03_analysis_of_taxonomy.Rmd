...
genomes_type <-
  genomes_type %>%
  anti_join(wrong_genomes_type)
```

Now we can make a table with "combinations" of genome clusters and names, to inspect taxonomic problems:
...
Now for our ultimate result of this part: a table with clusters named using automatically identified type genomes (tgs): 
...
bind_rows(
  ...
  mutate_at("names", str_replace_all, "Lactobacillus", "L.") %>%
  mutate_at("names", str_replace_all, "Pediococcus", "P.") %>%
  mutate_at("names", str_replace_all, "Weissella", "W.") %>%
  mutate_at("names", str_replace_all, "Leuconostoc", "Leuc.") %>%
  arrange(merger_split, desc(min_cni_within)) %>%
  write_csv(paste0(dout, "/table_1.csv"))
```

# Name genome clusters using 16S rRNA hits and NCBI labels

Let's make an overview of clusters not named used the type genomes approach: 
...
clusters_tss %>%
  ...
  mutate_all(str_replace_all, "Lactobacillus", "L.") %>%
  mutate_all(str_replace_all, "Pediococcus", "P.") %>%
  mutate_all(str_replace_all, "Weissella", "W.") %>%
  mutate_all(str_replace_all, "Leuconostoc", "Leuc.") %>%
  write_csv(paste0(dout, "/table_2.csv"))
```
...
ggsave(paste0(dout, "/reclassifications_unclassified.png"), units = "cm", width = 25, height = 20)
```
...
genomes_to_plot <- 
  ...
  mutate(is_new_species = str_detect(species_cni, "species")) %>%
  mutate(species_cni = fct_reorder(species_cni, is_new_species) %>% fct_rev()) %>%
...
(
  fig_reclassifications_classified <-
    genomes_to_plot %>%
    ggplot(aes(x = species_ncbi, y = species_cni, col = reclassified)) +
...
ggsave(paste0(dout, "/reclassifications_classified.png"), units = "cm", width = 25, height = 20)
```
...
ggsave(paste0(dout, "/figure_4.jpg"), units = "cm", width = 35, height = 40)
```
